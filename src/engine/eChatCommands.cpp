...
tString se_consoleCommand("/console");
...
bool LocalChatCommands(ePlayerNetID *p, tString command)
{
    auto commandFactory = CommandFactory();
    return LocalChatCommands(p, command, commandFactory);
}

bool RunChatCommand(ePlayerNetID *p, ChatCommand &command, tString args)
{
    try
    {
...
bool LocalChatCommands(ePlayerNetID *p, tString command, std::unordered_map<tString, std::function<std::unique_ptr<ChatCommand>()>> &commandFactories)
{
    if (p == nullptr)
    {
        return false;
    ...
    if (chatcmdFactory != commandFactories.end())
    {
        ...
        bool success = RunChatCommand(p, *chatcmd, arguments);
        if (!success)
