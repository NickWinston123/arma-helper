...
static void S_ZoneWallInteraction(eWall *pWall)
{
    ...
    REAL Vn = XY * V;
    if (Vn == 0)
        return; // if zone is moving along the wall, no impact ...
    REAL d = XY * XP;
    ...
    if (t > 0 || t < -s_zoneWallInteractionZoneDeltaTime)
        return;
...
bool gZone::Timestep(REAL time)
{
    ...
    if (sn_GetNetState() != nCLIENT)
    {
        ...
        if (targetRadius_ && (((GetExpansionSpeed() > 0) && (GetRadius() >= targetRadius_)) ||
...
static bool TriggerAvoidZone(gCycle *target, gZone *Zone, REAL currentTime)
{
    ...
    if ((!player->IsHuman()) || (sg_chatbotAvoidZones && player->IsChatting() && player->IsActive()))
    {
        ...
        for (int i = 41; i > 0; --i)
        {
            pX = (radi * cos(M_PI * 2 * (i / 42.f))) + zonX;
            pY = (radi * sin(M_PI * 2 * (i / 42.f))) + zonY;
...
static void TriggerZoneApproach(gCycle *target, gZone *Zone, REAL currentTime)
{
    ...
    if (target->Alive() && target->GetLastTurnTime() + target->GetTurnDelay() < currentTime)
    {
        ...
        {
            if (TriggerAvoidZone(target, Zone, currentTime))
                return;
        }
...
bool gZone::DoCycleInteract(gCycle *target, REAL time)
{
    ...
    REAL delta = b * b - 4 * a * c;
    if (delta < 0)
        return false; // no t. it can't be, an impact just occured ...
    delta = sqrt(delta);
    REAL t1 = (-b - delta) / (2 * a);
    REAL t2 = (-b + delta) / (2 * a);
    REAL t = 0;
    if ((t1 > 0) && (t2 > 0))
        return false; // can't be, again ...
    if (t1 > 0)
        t = t2;
    else if (t2 > 0)
        t = t1;
...
void gBaseZoneHack::OnEnter(gZone *target, REAL time)
{
    ...
    if (!team || !target)
        toScore = true;
    else if (sg_ballTeamMode)
        toScore = target->Team() == team ? 1 : 0;
    else
        toScore = target->Team() == team ? 0 : 1;
...
gZone &gZone::SetRadiusSmoothly(REAL radius, REAL expansionSpeed)
{
    ...
    if (!resizeRequested_)
        previousExpansionSpeed_ = GetExpansionSpeed();
...
{
    if ((newValue < gFLAGPASS_DISABLE) || (newValue > gFLAGPASS_PERSON))
        return false;
    return true;
}
...
void gFlagZoneHack::PassTheFlag(tString name)
{
    if (owner_)
    {
        if (sg_flagPassMode == gFLAGPASS_DISABLE)
            return;
            ...
            for (int i = 0; i < owner_->Team()->players.Len(); i++)
            {
                ...
                if (p && (p != owner_->Player()))
                {
                    ...
                    if (pCycle && pCycle->Alive())
                    {
                        ...
                        if (furthestDistance >= sg_flagPassDistance)
                            break;
                    }
                }
            }
...
bool gTargetZoneHack::Timestep(REAL time)
{
    ...
    if (((currentState_ == State_Conquering) && (sg_targetTimeBeforeVanishOnceConquered >= 0) && (time - timeFirstEntry_ > sg_targetTimeBeforeVanishOnceConquered)) || ((currentState_ != State_Conquered) && (zoneScore_ <= 0) && (zoneScore_ != zoneInitialScore_)))
    {
        // zone is conquered, make the zone collapse ...
        currentState_ = State_Conquered;
        SetReferenceTime();
        ...
        else
        {
            ...
            if (sg_soccerBallFirstWin && (target->ballShots_ == 1))
            {
                if (ownGoal)
                {
                    ...
                    if (!win)
                        sg_DeclareWinner(target->lastTeamIn_);
                }
                else
...
tArray<gCheckpointZoneHack *> sg_GetCheckpointZones()
{
    ...
    for (int i = 0; i < gameObjects.Len(); i++)
    {
        gCheckpointZoneHack *checkpointZone = dynamic_cast<gCheckpointZoneHack *>(gameObjects[i]);
        if (checkpointZone)
            checkpointZones.Insert(checkpointZone);
    }
...
static void sg_CreateZone_conf(std::istream &s)
{
    ...
    if (zoneTypeStr == "teleport")
    {
        ...
        if (isInter || zoneRelAbsStr == "")
            zoneRelAbsStr = "rel";
        if (zoneRelAbsStr == "rel")
            relJump = 1;
        else if (zoneRelAbsStr == "cycle")
            relJump = 2;
        else
            relJump = 0;
...
static void sg_SetZoneCoord(std::istream &s, bool byId)
{
    ...
    REAL posX, posY;
    s >> posX;
    posX *= gArena::SizeMultiplier();
    s >> posY;
    posY *= gArena::SizeMultiplier();
...
static void sg_SetZoneDir(std::istream &s, bool byId)
{
    ...
    REAL dirX, dirY;
    s >> dirX;
    dirX *= gArena::SizeMultiplier();
    s >> dirY;
    dirY *= gArena::SizeMultiplier();
