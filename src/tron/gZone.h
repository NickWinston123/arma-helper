...
public:
    ...
    gZone &SetWallInteract(bool wallInteract)
    {
        wallInteract_ = wallInteract;
        return *this;
    }
    gZone &SetWallBouncesLeft(int wallBouncesLeft)
    {
        wallBouncesLeft_ = wallBouncesLeft;
        return *this;
    }
    bool GetWallInteract() { return wallInteract_; }
    int GetBouncesLeft() { return wallBouncesLeft_; }

    gZone &SetCycleInteract(bool cyclInteract)
    {
        cycleInteract_ = cyclInteract;
        return *this;
    }
...
public:
    void SetOnEnterCmd(tString &cmd, tString &mode)
    {
        if (mode == "add")
            OnEnterCmd << "\n"
                       << cmd;
    ...
    void SetOnExitCmd(tString &cmd, tString &mode)
    {
        if (mode == "add")
            OnExitCmd << "\n"
                      << cmd;
    ...
    void SetOnVanishCmd(tString &cmd, tString &mode)
    {
        if (mode == "add")
            OnVanishCmd << "\n"
                        << cmd;
...
public:
    ...
    void SetJump(eCoord coord, int rel)
    {
        this->jump = coord;
        this->relative = rel;
    }
